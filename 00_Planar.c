/*
*						 
*	Project #1: Generate a Planar Subgraph  
*						 
...
Part 2 . Remove the line where exist other nodes in the closed disc.

Step :

Part 1.
1.Design two structure to record point and line data.
2.Read file to store point (x,y) and call mathematis formula to calculate the distance between any two point.

Part 2.
3.Fixed line and make circle (find middle of line and radius) to test wheather is node inside circle or not.
4.Get set of repeated point, and delete them.

Summary :

I don't have better method to solve the problem, just use for loop to visit all nodes again, so I think my time complexity is not efficiently.

Most of time to run two level of loop, the time complexity is O(n^2).
