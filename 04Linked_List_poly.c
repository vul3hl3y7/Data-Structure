...
2.Use command "./hwc" to run program.

For the strat, you will see the menu in the following :

===========================

Please enter the file of polynomial data.

Polynomial 1 :
Polynomial 2 :

And after enter the file, it will show
A(x) = polynomial 1
B(x) = polynomial 2
C(x) = polynomial 1 + 2
D(x) = polynomial 1 - 2
...
(1)If data file doesn't exist, program will ask for the file utill file exist.

(2)file type should be a pair of "coef(int/float) exp(int)" in one line, or function won't correct display.

(3)In the entire program, I only show the second digit after thr decimal point of the coefficient.
