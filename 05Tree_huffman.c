...
Part 1 . Based on lengths of assigned codes based on frequencies
Part 2 . Encrypted code to a file


Step :

Part 1.
1.Design heap structure to record character, frequency, and huffman code array.
2.Read sentence in file , and calculate number of different characters. 
¡÷ countStrfrequency
Use 128 ASCII order.

Part 2.
3.According to frequency of different characters to inset heap tree.
4.Adjust to min-heap tree.
5.According to min-heap tree extract and insert to build Huffman tree.

createMin_heap¡÷buildMin_heap¡÷Min_heapify
¡÷insertMin_heap && extractMin¡÷createAndBuildMin_heap¡÷buildHuffmanTree

Part 3.
5.According to Huffman tree, use leaf and high to set huffman code for all nodes.
¡÷printCodes
set 0 to left subtree , set 1 to right subtree until leaf end.
6.Calculate how many bits of huffman code.
7.Store huffman code array to structure.

Part 4.
7.Creat a tree list to record all characters and their huffman code array.
8.According to sentence of reading file, convert character one by one to huffman code and display. 
